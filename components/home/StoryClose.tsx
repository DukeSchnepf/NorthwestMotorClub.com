 ...
 * with rust-dashed callouts inviting Duke to wire real counts.
 */
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at 80% 80%, rgba(255,122,26,0.05) 0%, transparent 50%)",
        }}
      />
      ...
      <div className="relative mx-auto grid max-w-content gap-12 lg:grid-cols-[1fr_1.2fr] lg:items-center">
        ...
        <div>
          ...
          <h3 className="mt-4 font-display text-4xl font-normal leading-[1.05] tracking-[-0.025em] text-fg md:text-5xl lg:text-6xl">
            Still chasing the <span className="italic text-lime">long way</span>{" "}
            home.
          </h3>
          <blockquote className="mt-6 max-w-[48ch] border-l-2 border-rust pl-5 font-display text-lg italic leading-relaxed text-fog md:text-xl">
        ...
        <div className="grid grid-cols-2 gap-4 border border-line bg-raised p-6 md:p-8">
          <Stat
            ...
            accent="lime"
          />
          <Stat n="12" label="Bays in the roster" accent="rust" />
          <Stat n="047" label="Drives logged" accent="lime" placeholder />
          <Stat n="∞" label="Backroads left" accent="lime" />
        </div>
      </div>
    </section>
  );
    ...
    <div>
      <div
        className={`font-display text-5xl font-normal leading-none tracking-[-0.04em] ${
          accent === "rust" ? "text-rust" : "text-lime"
        }`}
      >
        {n}
      </div>
      ...
      {placeholder && (
        <div className="mt-1 font-mono text-[0.5rem] uppercase tracking-[0.2em] text-rust/80">
          ◯ wire real count
        </div>
      )}
    </div>
